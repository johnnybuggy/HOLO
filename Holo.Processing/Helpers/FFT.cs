    ...
    public class FFTCalculator
    {
        ...
        public void FFT(float[] data, bool forward)
        {
            ...
            while (n > mmax)
            {
                ...
                float wr = 1, wi = 0;
                var wpr = (float)(float)Math.Cos(theta);
                var wpi = (float)(float)Math.Sin(theta);
        ...
        public float[] Norm(float[] complexArray)
        {
            var res = new float[complexArray.Length/2];
            for(int i=0;i<complexArray.Length;i+=2)
            {
                var a = complexArray[i];
                var b = complexArray[i + 1];
                res[i/2] = (float)Math.Sqrt( a*a + b*b);
            }
